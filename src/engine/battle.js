...
export function diagnoseBattle(b) {
  ...
  if (myDef && myDef.max > DEF_CP_BASE) {
    verdict.push(`Оборону удавалось расширить до ${myDef.max} — это дало Знамя.`);
  } else if (myDef) {
    verdict.push('Оборона всю игру закрывала одну угрозу за раунд: больше не позволял бюджет.');
    advice.push('Оборону расширяет Знамя; Тактик держит двоих за одно очко, а Муштра блокирует бесплатно.');
  }
